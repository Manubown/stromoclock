...
import { fetchDayAheadPrices, findAdaptiveWindow, providerFor } from "@/lib/prices";
...
export default async function DashboardPage() {
  ...
  const cheapWindow = prices && prices.length > 0 ? findAdaptiveWindow(prices, "cheapest") : null;
  const expensiveWindow =
    prices && prices.length > 0 ? findAdaptiveWindow(prices, "most-expensive") : null;
  ...
  return (
    <main className="mx-auto max-w-4xl px-4 py-6 sm:px-6 sm:py-10">
      <header className="mb-6 flex items-center justify-between sm:mb-8">
        ...
        <Link
          href="/settings"
          className="rounded-md border border-line bg-panel px-3 py-1.5 text-sm text-muted hover:border-accent hover:text-ink"
        >
          Settings
        </Link>
      </header>
      ...
      {fetchError && (
        <p className="mb-4 rounded-md border border-danger/40 bg-panel p-4 text-sm text-danger">
      ...
      {prices && prices.length > 0 && (
        <section>
          <div className="mb-4 grid grid-cols-2 gap-2 sm:grid-cols-4 sm:gap-3">
            ...
            {cheapWindow && (
              <Stat
                label={`Cheap ${cheapWindow.hours}h window`}
            ...
            {expensiveWindow && (
              <Stat
                label={`Expensive ${expensiveWindow.hours}h window`}
